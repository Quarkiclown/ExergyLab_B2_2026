%% Energy and Exergy Analysis: Professional Dashboard (Cold Flow Var)
...
% Data organized as matrices: [Point 1; Point 2; Point 3; Point 4]
% Parallel Flow Experimental Matrix
expDataP = [
% Cols: Flow_H, Flow_C, Thi,   Tho,   Tci,  Tco
        823.05, 447.83, 28.96, 27.47, 6.42, 9.08;
        838.20, 757.97, 29.10, 27.23, 6.76, 8.73;
        838.95, 1099.2, 29.42, 26.98, 7.00, 8.73;
        831.00, 1386.0, 29.18, 26.88, 7.29, 8.70
];
...
% Cols: Flow_H, Flow_C, Thi,   Tho,   Tci,  Tco
        ...
        811.65, 1099.4, 29.14, 26.41, 7.18, 9.21;
        811.95, 1398.4, 29.50, 26.39, 7.76, 9.53
];
...
% Simulation Setup: Predictive model across cold flow range
...
mh_mod = (mean([P.flow_h, C.flow_h])/3600) * (rho_h_mod/1000);
mc_mod_vec = (fc_pred/3600) * (rho_c_mod/1000);
ratio_pred = mh_mod ./ mc_mod_vec;
...
[P_mod_I, P_mod_psi, P_mod_epsEx] = runValidatedModel(fc_pred, mean(P.flow_h), mean(P.Thi), mean(P.Tci), 'parallel', T0, nt, di, do, L, Ds, k_w, k_s, mu_const, Pr);
[C_mod_I, C_mod_psi, C_mod_epsEx] = runValidatedModel(fc_pred, mean(C.flow_h), mean(C.Thi), mean(C.Tci), 'counter', T0, nt, di, do, L, Ds, k_w, k_s, mu_const, Pr);
...
%% --- FIGURE 1: Performance vs. Volumetric Cold Flow ---
figure('Name', 'Exergy Analysis: Flow Rate', 'Position', [50, 50, 800, 950], 'Color', 'w');
t1 = tiledlayout(3, 1, 'TileSpacing', 'compact', 'Padding', 'loose');
title(t1, '\textbf{Exergy Performance vs. Cold Flow Rate (Cold Flow Var)}', 'Interpreter', 'latex', 'FontSize', 16);

ax1 = nexttile; hold on;
p_mod1 = plot(fc_pred, P_mod_I, '--', 'Color', [colorP opacity], 'LineWidth', 2);
c_mod1 = plot(fc_pred, C_mod_I, '--', 'Color', [colorC opacity], 'LineWidth', 2);
p_exp1 = plotExp(ax1, P.flow_c, P.I, colorP, 'o'); 
c_exp1 = plotExp(ax1, C.flow_c, C.I, colorC, 's');
...
plot(fc_pred, P_mod_psi, '--', 'Color', [colorP opacity], 'LineWidth', 2);
plot(fc_pred, C_mod_psi, '--', 'Color', [colorC opacity], 'LineWidth', 2);
plotExp(ax2, P.flow_c, P.psi, colorP, 'o'); plotExp(ax2, C.flow_c, C.psi, colorC, 's');
ylabel('$\psi$ [\%]', 'Interpreter', 'latex'); stylePlot(ax2, 0:10:50, [0 50]);

ax3 = nexttile; hold on;
plot(fc_pred, P_mod_epsEx, '--', 'Color', [colorP opacity], 'LineWidth', 2);
plot(fc_pred, C_mod_epsEx, '--', 'Color', [colorC opacity], 'LineWidth', 2);
plotExp(ax3, P.flow_c, P.epsEx, colorP, 'o'); plotExp(ax3, C.flow_c, C.epsEx, colorC, 's');
ylabel('$\epsilon_{ex}$ [\%]', 'Interpreter', 'latex'); xlabel('Cold Fluid Flow Rate [l/h]', 'Interpreter', 'latex');
stylePlot(ax3, 0:10:50, [0 50]);
linkaxes([ax1, ax2, ax3], 'x'); xlim(ax1, [0 2000]);

%% --- FIGURE 2: Performance vs. Mass Flow Ratio ---
figure('Name', 'Exergy Analysis: Mass Ratio', 'Position', [860, 50, 800, 950], 'Color', 'w');
t2 = tiledlayout(3, 1, 'TileSpacing', 'compact', 'Padding', 'loose');
title(t2, '\textbf{Exergy Performance vs. Mass Flow Ratio (Cold Flow Var)}', 'Interpreter', 'latex', 'FontSize', 16);
...
linkaxes([ax4, ax5, ax6], 'x'); xlim(ax4, [0 4.5]);
...
function [I, psi, epsilon_ex, Q] = calcExergy(fh, fc, Thi, Tho, Tci, Tco, T0)
    % Thermodynamic Exergy Balance based on flow inlet/outlet
    ...
    Q = mh.*cp_h.*(ThiK - ThoK); % Energy Transfer [W]
    dExH = mh.*cp_h.*((ThiK - ThoK) - T0.*log(ThiK./ThoK)); % Hot Exergy Change
    dExC = mc.*cp_c.*((TcoK - TciK) - T0.*log(TcoK./TciK)); % Cold Exergy Change
...
function [I, psi, epsilon_ex] = runValidatedModel(fc_list, fh, Thi, Tci, mode, T0, nt, di, do, L, Ds, k_w, k_s, mu, Pr)
    % Predictive Model using Convection Correlations (Dittus-Boelter & McAdams)
    I = zeros(size(fc_list)); psi = zeros(size(fc_list)); epsilon_ex = zeros(size(fc_list));
    ...
    for i = 1:length(fc_list)
        ...
        mh = (fh/3600)*(rho_h/1000); mc = (fc_list(i)/3600)*(rho_c/1000);
        
        % Calculation of Heat Transfer Coefficients (h) via Nusselt (Nu)
        ...
        % Determination of Effectiveness (eps) using NTU Method
        ...
        % Heat rate and outlet temperature prediction
